...
class MessageLookup extends MessageLookupByLibrary {
  ...
  static m0(date) => "开发组公告 ${date}";

  static m1(time) => "将在${time}秒后自动打卡，点击以取消";

  static m2(mostCrowded, leastCrowded) =>
      "[排队最多]${mostCrowded}餐厅 [排队最少]${leastCrowded}餐厅";

  static m3(name) => "回复 No.${name}";

  static m4(week) => "第 ${week} 周";

  static m5(name) => "欢迎你，${name}";
  ...
  static _notInlinedMessages(_) => <String, Function>{
    ...
    "login_issue_1" : MessageLookupByLibrary.simpleMessage("登录失败，旦兮无法完成 UIS 登录。\n出现此错误，很可能是由于您连续多次登录失败所致。\n您需要使用浏览器手动完成一次登录，从下一次登录开始，旦兮 即可妥善处理此情况。"),
        ...
        "login_uis_description":
            MessageLookupByLibrary.simpleMessage("您的用户名和密码仅会被用于登录复旦UIS"),
        ...
        "logout_question_prompt":
            MessageLookupByLibrary.simpleMessage("改动将在app退出后生效"),
